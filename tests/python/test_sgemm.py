...
def run_sgemm_case(
    ...
    *,
    ...
    out = cuda_ops.sgemm(a, b, alpha, beta)
    assert_close_with_stats(name, out, ref, atol=atol, rtol=rtol)

    if check_dispatcher:
        out_ops = torch.ops.cuda_ops.gemm(a, b, alpha, beta)
...
OUT = cuda_ops.sgemm(A, B, 1.0, 0.0)
assert_close_with_stats("cuda_ops.sgemm.smoke", OUT, REF, atol=1e-3, rtol=1e-5)

OUT_OPS = torch.ops.cuda_ops.gemm(A, B, 1.0, 0.0)
assert_close_with_stats("torch.ops.cuda_ops.gemm.smoke", OUT_OPS, REF, atol=1e-3, rtol=1e-5)

A_grad = A.detach().clone().requires_grad_(True)
B_grad = B.detach().clone().requires_grad_(True)
loss = cuda_ops.sgemm(A_grad, B_grad).sum()
loss.backward()
...
compiled = torch.compile(lambda x, y: cuda_ops.gemm(x, y), fullgraph=True)
out_compiled = compiled(A, B)
assert_close_with_stats("torch.compile(cuda_ops.gemm)", out_compiled, REF, atol=1e-3, rtol=1e-5)
