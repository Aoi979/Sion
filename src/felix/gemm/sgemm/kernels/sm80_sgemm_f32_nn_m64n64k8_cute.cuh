...
__global__ static __launch_bounds__(
    decltype(size(CThreadLayout{}))::
        value * 2) void sm80_sgemm_f32_nn_m64n64k8_cute_kernel(ProblemShape shape_MNK,
                                          CtaTiler cta_tiler,
  ...
  ////////////////////////////////////////////////////////////////////////
  Tensor swC = local_tile(sC, iter_tiler, warp_coord);      //(16, 32)
  Tensor slC = local_tile(swC, lane_tiler, lane_coord); //(4, 4)
  ////////////////////////////////////////////////////////////////////////
  ...
  auto rA_Layout = make_layout(make_shape(Int<4>{}, Int<2>{}, Int<2>{}));
  Tensor rA = make_tensor<float>(rA_Layout);
  ...
  auto rB_Layout = make_layout(make_shape(Int<4>{}, Int<2>{}, Int<2>{}));
  Tensor rB = make_tensor<float>(rB_Layout);

  // Copy gmem to rmem for k_tile=0
  copy(tAgA(_, _, 0), tArA);
  copy(tBgB(_, _, 0), tBrB);
  // rmem -> smem ///////////////////// bank conflict
  ...
  for (int k_tile = 1; k_tile < K_TILE_MAX; ++k_tile) {
    ...
    for (int k_block = 1; k_block < K_BLOCK_MAX; k_block++) {
      copy(slA(_, _, k_block, load_stage_idx), rA(_, _, (k_block % 2)));
      copy(slB(_, _, k_block, load_stage_idx), rB(_, _, (k_block % 2)));
      CUTE_UNROLL
      for (int m = 0; m < 2; ++m) {
        CUTE_UNROLL
        for (int n = 0; n < 2; ++n) {
          gemm(rA(_, m, ((k_block - 1) % 2)), rB(_, n, ((k_block - 1) % 2)),
               rC(_, _, m, n));
        }
      }
    }
    ...
    for (int m = 0; m < 2; ++m) {
      CUTE_UNROLL
      for (int n = 0; n < 2; ++n) {
        gemm(rA(_, m, ((K_BLOCK_MAX - 1) % 2)),
             rB(_, n, ((K_BLOCK_MAX - 1) % 2)), rC(_, _, m, n));
      }
    }
  }
  ...
  for (int m = 0; m < 2; ++m) {
    CUTE_UNROLL
    for (int n = 0; n < 2; ++n) {
      gemm(rA(_, m, ((K_BLOCK_MAX - 1) % 2)), rB(_, n, ((K_BLOCK_MAX - 1) % 2)),
           rC(_, _, m, n));
    }
  }
  //////////////    Epilogue    ////////////////////
  {
    for (int m = 0; m < 2; ++m) {
      for (int n = 0; n < 2; ++n) {
        copy(rC(_, _, m, n), slC);
        auto tCsC = local_partition(swC, tC, lane_id);
        auto gwIterC_s = gwIterC(_,_,m,n);
        auto tCgC = local_partition(gwIterC_s, tC, lane_id);
