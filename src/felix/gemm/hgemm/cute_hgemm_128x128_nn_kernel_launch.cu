...
FelixStatus hgemm_nt(uint32_t m, uint32_t n, uint32_t k, float alpha, cute::half_t const *A,
              cute::half_t const *B, float beta, cute::half_t *C,
...
// void hgemm_nn(int m, int n, int k, Alpha alpha, cute::half_t const *A, int ldA,
...
//   TiledCopy copyB = make_tiled_copy(
//       Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS<uint128_t>, cute::half_t>{},
//       Layout<Shape<_16, _8>, Stride<_1, _16>>{}, // Thr layout 16x8 k-major
...
REGISTER_KERNEL(cute_hgemm_128x128_nn,
                (felix::KernelEntry{felix::KernelType::HGEMM,
                                    "cute_hgemm_128x128_nt",
                                    (void *)felix::hgemm_nt, false}));
