...
void launch_flash_attn_mma_stages_1D(const torch::Tensor &Q,const torch::Tensor &K,
    ...
    const uint32_t smem_size = (Q_smem_size + K_stages_smem_size + V_smem_size) * sizeof(at::Half);
    ...
    cudaFuncSetAttribute(ampere_flash_attn_mma16168_64_1D_warp_tiling<HEAD_DIM, MMA_M, MMA_N, MMA_K, STAGE>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);

    ampere_flash_attn_mma16168_64_1D_warp_tiling<HEAD_DIM, MMA_M, MMA_N, MMA_K, STAGE>
            <<<grid, block, smem_size>>>(dQ, dK, dV, dO, H, N);
...
template void launch_flash_attn_mma_stages<128,2>(const torch::Tensor &Q,const torch::Tensor &K,
                                  const torch::Tensor &V, torch::Tensor &O);
template void launch_flash_attn_mma_stages_1D<64,2>(const torch::Tensor &Q,const torch::Tensor &K,
                                  const torch::Tensor &V, torch::Tensor &O);
template void launch_flash_attn_mma_stages_1D<128,2>(const torch::Tensor &Q,const torch::Tensor &K,
                                  const torch::Tensor &V, torch::Tensor &O);                                    
