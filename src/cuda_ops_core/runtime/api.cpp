...
Status current_device_capability(DeviceCapability &device_capability) {
  int device = 0;
  cudaError_t err = cudaGetDevice(&device);
  if (err != cudaSuccess) {
    return Status::make(Status::Type::API_ERROR, err,
                             "cudaGetDevice failed during CudaOpsCore dispatch");
  }
  ...
  if (err != cudaSuccess) {
    return Status::make(
        Status::Type::API_ERROR, err,
        "cudaGetDeviceProperties failed during CudaOpsCore dispatch");
  }
...
Status no_matching_gemm_kernel(const GemmDispatchKey &key) {
  std::ostringstream oss;
  oss << "No matching CudaOpsCore GEMM kernel found"
  ...
  return Status::make(Status::Type::API_ERROR, cudaErrorInvalidValue,
                           oss.str());
}

Status no_matching_flash_attn_kernel(const FlashAttnDispatchKey &key) {
  std::ostringstream oss;
  oss << "No matching CudaOpsCore FlashAttention kernel found"
      << " cc=" << key.device.cc
      << " max_dynamic_smem=" << key.device.max_dynamic_smem_bytes
      << " max_threads_per_block=" << key.device.max_threads_per_block
...
Status wrong_named_kernel_type(const KernelEntry &entry,
                                    KernelType expected,
...
Status missing_named_kernel(const std::string &kind,
                                 const std::string &name) {
  return Status::make(Status::Type::API_ERROR, cudaErrorInvalidValue,
...
Status named_kernel_not_supported(const KernelEntry &entry,
...
Status dispatch_sgemm_entry(const KernelEntry &entry, uint32_t M,
...
Status dispatch_hgemm_entry(const KernelEntry &entry, uint32_t M,
                                 uint32_t N, uint32_t K, float alpha,
...
Status dispatch_flash_attn_entry(const KernelEntry &entry, half *Q,
...
Status dispatch_topk_entry(const KernelEntry &entry, float const *data,
...
Status sgemm_f32_launch(uint32_t M, uint32_t N, uint32_t K, float alpha,
                             float const *A, float const *B, float beta,
                             float *C, cudaStream_t stream) {
  DeviceCapability device{};
...
Status hgemm_f16_launch(uint32_t M, uint32_t N, uint32_t K, float alpha,
...
Status hgemm_f16_nt_launch(uint32_t M, uint32_t N, uint32_t K, float alpha,
                                half const *A, half const *B, float beta,
                                half *C, cudaStream_t stream) {
  DeviceCapability device{};
  auto status = current_device_capability(device);
...
Status topk_f32_radix_select_launch(float const *data, float *out,
                                         uint32_t num_slices,
...
template <>
Status flash_attn_f16_launch<64, 64>(half *Q, half *K, half *V, half *O,
...
template <>
Status flash_attn_f16_launch<128, 64>(half *Q, half *K, half *V, half *O,
                                           uint32_t heads, uint32_t batch_size,
                                           uint32_t QKV_seqlen,
                                           cudaStream_t stream) {
  DeviceCapability device{};
...
template <>
Status flash_attn_f16_launch_by_name<64, 64>(
...
template <>
Status flash_attn_f16_launch_by_name<128, 64>(
    half *Q, half *K, half *V, half *O, uint32_t heads, uint32_t batch_size,
    uint32_t QKV_seqlen, cudaStream_t stream, const std::string &kernel_name) {
  DeviceCapability device{};
  Status status;
