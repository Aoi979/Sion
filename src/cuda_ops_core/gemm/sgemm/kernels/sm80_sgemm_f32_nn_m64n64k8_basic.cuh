...
__global__ void sm80_sgemm_f32_nn_m64n64k8_basic_kernel(uint32_t M, uint32_t N, uint32_t K,
                                      float alpha, float const *__restrict__ A,
                                      float const *__restrict__ B, float beta,
                                      float *__restrict__ C) {
  using Config = cuda_ops_core::Sgemm64x64Config;
  using ThreadInfo = cuda_ops_core::ThreadInfo<Config>;
  using BlockOffset = cuda_ops_core::BlockOffset<Config>;
  using SharedStorage = cuda_ops_core::SharedStorage<Config>;
  using GmemIteratorA = cuda_ops_core::GmemIteratorA<Config>;
  using GmemIteratorB = cuda_ops_core::GmemIteratorB<Config>;
  using WarpMma = cuda_ops_core::WarpMma<Config>;
  using MmaPipeline = cuda_ops_core::MmaPipeline<Config>;
  using Epilogue = cuda_ops_core::Epilogue<Config>;
