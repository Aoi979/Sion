...
namespace cuda_ops_core::detail::sm90::wgmma {

__device__ __forceinline__ uint64_t encode_matrix_descriptor(uint64_t value) {
  return (value & 0x3ffff) >> 4;
}

__device__ __forceinline__ uint64_t make_k_major_smem_descriptor(half *ptr) {
  uint32_t addr = static_cast<uint32_t>(__cvta_generic_to_shared(ptr));
  uint64_t desc = encode_matrix_descriptor(addr);
  desc |= encode_matrix_descriptor(16) << 16;
  desc |= encode_matrix_descriptor(1024) << 32;
  desc |= 1ull << 62;
  return desc;
}

template <int BlockN>
__device__ __forceinline__ uint64_t make_b_mn_major_smem_descriptor(half *ptr) {
  uint32_t addr = static_cast<uint32_t>(__cvta_generic_to_shared(ptr));
  uint64_t desc = encode_matrix_descriptor(addr);
  desc |= encode_matrix_descriptor(1024) << 16;
  desc |= encode_matrix_descriptor(static_cast<uint64_t>(BlockN) * 16) << 32;
  desc |= 1ull << 62;
  return desc;
}
...
__device__ __forceinline__ void wgmma_m64n128k16_f32_f16_f16(float d[8][8],
                                                              half *sA,
                                                              half *sB) {
  uint64_t desc_a = make_k_major_smem_descriptor(sA);
  uint64_t desc_b = make_b_mn_major_smem_descriptor<128>(sB);
  asm volatile(
      "{\n"
      ".reg .pred p;\n"
      "setp.ne.b32 p, %66, 0;\n"
...
template <int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ __forceinline__ void wgmma_m64n256k16_f32_f16_f16(
    float d[16][8], half *sA, half *sB) {
  uint64_t desc_a = make_k_major_smem_descriptor(sA);
  uint64_t desc_b = make_b_mn_major_smem_descriptor<256>(sB);
  asm volatile("{\n"
               ".reg .pred p;\n"
               "setp.ne.b32 p, %130, 0;\n"
