...
def _check_gemm_contract(a: torch.Tensor, b: torch.Tensor, *, nt: bool = False):
    ...
    torch._check(
        ...
        lambda: "CudaOps GEMM supports float16 and float32 tensors",
    )
    if nt:
...
@torch.library.register_fake("cuda_ops::gemm")
def _fake_gemm(a, b, alpha=1.0, beta=0.0):
    shape = _check_gemm_contract(a, b)
    return torch.empty(shape, device=a.device, dtype=a.dtype)


@torch.library.register_fake("cuda_ops::sgemm")
def _fake_sgemm(a, b, alpha=1.0, beta=0.0):
    shape = _check_gemm_contract(a, b)
    torch._check(a.dtype == torch.float32, lambda: "cuda_ops::sgemm requires float32 tensors")
    return torch.empty(shape, device=a.device, dtype=a.dtype)


@torch.library.register_fake("cuda_ops::hgemm")
def _fake_hgemm(a, b, alpha=1.0, beta=0.0):
    shape = _check_gemm_contract(a, b)
    torch._check(a.dtype == torch.float16, lambda: "cuda_ops::hgemm requires float16 tensors")
    return torch.empty(shape, device=a.device, dtype=a.dtype)


@torch.library.register_fake("cuda_ops::hgemm_nt")
def _fake_hgemm_nt(a, b, alpha=1.0, beta=0.0):
    shape = _check_gemm_contract(a, b, nt=True)
    torch._check(a.dtype == torch.float16, lambda: "cuda_ops::hgemm_nt requires float16 tensors")
    return torch.empty(shape, device=a.device, dtype=a.dtype)


@torch.library.register_fake("cuda_ops::flash_attention")
def _fake_flash_attention(query, key, value):
    ...
    torch._check(query.dtype == torch.float16, lambda: "cuda_ops::flash_attention requires float16 tensors")
    torch._check(key.dtype == torch.float16, lambda: "cuda_ops::flash_attention requires float16 tensors")
    torch._check(value.dtype == torch.float16, lambda: "cuda_ops::flash_attention requires float16 tensors")
...
torch.library.register_autograd(
    "cuda_ops::gemm", _gemm_backward, setup_context=_setup_gemm_context
)
torch.library.register_autograd(
    "cuda_ops::sgemm", _gemm_backward, setup_context=_setup_gemm_context
)
torch.library.register_autograd(
    "cuda_ops::hgemm", _gemm_backward, setup_context=_setup_gemm_context
)
torch.library.register_autograd(
    "cuda_ops::hgemm_nt", _gemm_nt_backward, setup_context=_setup_gemm_nt_context
)
torch.library.register_autograd(
    "cuda_ops::flash_attention",
